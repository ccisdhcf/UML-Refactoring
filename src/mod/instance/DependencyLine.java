...
//		g.drawPolygon(polygon);
		...
		double tan = (double) (fpPrime.getY() - tpPrime.getY()) / (fpPrime.getX() - tpPrime.getX());
		double arctan = Math.toDegrees(Math.atan(tan));

		int sign = tpPrime.getX() > fpPrime.getX() ? -1 : 1;
		double arrowX = (arrowLen * Math.cos(Math.toRadians(arctan + arrowDegree)));
		double arrowY = (arrowLen * Math.sin(Math.toRadians(arctan + arrowDegree)));
		g.drawLine((int) tpPrime.getX(), (int) tpPrime.getY(), (int) (tpPrime.getX() + sign * arrowX),
				(int) (tpPrime.getY() + sign * arrowY));
...
//		g.drawLine(tpPrime.getX(), tpPrime.getY(), tpPrime.getX() + sign * arrowX, tpPrime.getY() + sign * arrowY);
	}
